...
delegate OnMouseDeClicked(optional GUIComponent Component = self, optional byte ButtonNum);
...
event MouseClicked(optional byte ButtonNum)
{
    ...
    if(bCanHaveClickSelection)
        bIsClickSelected = True;
}

event MouseDeClicked(optional byte ButtonNum)
{
    OnMouseDeClicked(self, ButtonNum);

    bIsClickSelected = False;
}
