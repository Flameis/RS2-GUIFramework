...
var() bool bStretchToMatch; // This will stretch the items to match the base component's width
        ...
        for (i = 0; i < ItemList.Length; i++)
        {
            ...
            if (ItemCaptionInfo != None)
            {
                ...
                if (IsSelectedItem(X, XL, YEnd + (YExtension * i), YExtension))
                {
                    ...
                    if(bStretchToMatch)
                        C.SetPos(GetComponentStartPos(C).X,Y);
...
final function bool IsSelectedItem(float X, float XL, float Y, float YL)
{
...
event MouseClicked(optional byte ButtonNum)
{
    ...
    Index = (ForcedCaptionInfo != 255) ? ForcedCaptionInfo : 0;
    if(SelectedItemIndex > -1 && Caption.Length > 0)
    {
        if(Caption[Index] != ItemList[SelectedItemIndex])
            OnPropertyChanged(self);

        Caption[Index] = ItemList[SelectedItemIndex];
    }
    
    super.MouseClicked(ButtonNum);
}

DefaultProperties
{
    bCanHaveClickSelection = true // Dropdown is activated while the component is click-selected
    SelectedItemIndex = -1 // Start without any item selected
}